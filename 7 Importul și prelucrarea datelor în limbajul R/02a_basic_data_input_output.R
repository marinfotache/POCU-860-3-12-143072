...
#################################################################################
##              Import data from PostgreSQL database "chinook" 
...
track <- dbReadTable(con, "track" )
genre <- dbReadTable(con, "genre" )
mediatype <- dbReadTable(con, "mediatype" )
playlist <- dbReadTable(con, "playlist" )
playlisttrack <- dbReadTable(con, "playlisttrack" )
invoiceline <- dbReadTable(con, "invoiceline" )
invoice <- dbReadTable(con, "invoice" )
customer <- dbReadTable(con, "customer" )
employee <- dbReadTable(con, "employee" )


###  D. Query (from R) the PostgreSQL database and import the query result as a data frame
albums_led_zepp <- dbGetQuery(con, 
...
View(albums_led_zepp)

# display first rows of the result
head(albums_led_zepp,5)
...
save.image(file = 'chinook_v2.RData')
